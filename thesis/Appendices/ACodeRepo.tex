...
\begin{table}[h!]
	...
	\begin{tabular}{|l |l | p{.4\textwidth}|} 
		...
		\hline
		\multirow{5}{*}{Controlplane/} & kubeadm\_config.yaml & The configuration which should be passed during initialization of the cluster using kubeadm. \\ 
		& open\_twin.yaml & The \ac{K8s} configuration file for deploying the cluster. \\ 
		\hline
		\multirow{2}{*}{Kubernetes/} & open\_twin.yaml & The Kubernetes configuration file for deploying the cluster. \\
		\hline
	\end{tabular}
	\caption{List of files in the code repository and their usage description.}
