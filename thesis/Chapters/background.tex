...
The system consists of the following micro services that are permanently accessible: \acf{GSS}, \acf{AUTH} and the database. The database is running on MongoDB\footnote{MongoDB:~https://www.mongodb.com/}. Another Service is the \acf{LSS} that spawns the so called compute services. Those are services for running the actual computation after opening a projet. logging, scripting, 3D modeling, kriging. In total there are XXX compute services, that can dynamically spawn and exit.
A list of the compute services can be found in Table XXXX.
...
\begin{table}[h!]
	...
	\begin{tabular}{||c c||} 
		\hline
		Name & Short description \\ [0.5ex] 
		\hline\hline
		CartesianMeshService & 6 \\ 
		FITTDService & 7 \\
		KrigingService & 545 \\ 
		LoggerService & 545 \\ 
		ModelingService & 545 \\ 
