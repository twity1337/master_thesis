...
\begin{figure}[hb]
	\centering
	\includegraphics[width=.98\textwidth]{Figures/pause_batch_file.png}
	\caption{Application errors in verbosity mode, shown with \ac{GSS} and \ac{LSS}. The user is now able to read the errors, since the processes are started in new windows and wait for user interaction.}
...
\begin{figure}[h]
	...
	\caption{Output of \tcode{kubectl get nodes} command after adding a Windows node to the cluster. The nodes are ready to run pods.}
