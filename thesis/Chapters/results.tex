...
Container images must have the same operating system as basis for their processes. Compared to virtualization this reduces the isolation on the one hand, but improves performance on the other.
...
Microsoft provides several image tags (\enquote{ltsc2019}, \enquote{ltsc2022}, and so on) for different kinds of operating system versions\cite{MattbriggsMicrosoft.20230214,Microsoft.20230313}. They also offer one general image tag for multiple architectures. However, they do not offer a more generous tag having a broad version coverage, even though the \acp{CRI} would support it. Therefore the host operating system always has to match the container image tag.

There are two different ways to solve this flaw of operating system inconsistency. One method is given by \ac{K8s}. \ac{K8s} supports filtering based on the build number of a node\cite{Brasmithms.20230313}. For this, a separate \tcode{nodeSelector} key is defined as shown in \autoref{lst:results.windows_build}.
...
\end{enumerate}
Since the application images of \ac{OT} become very large (\~{}2.8 Gigabyte), the three steps taking about five minutes each. Depending on the amount of container images to process and the performance of the host computer, this consumes about half an hour.
Therefore, to automate this process for a batch of container files, the PowerShell script \textit{build-image.ps1} has been created. The script reads Container files in a directory and afterwards performs the described steps on it as shown in \autoref{lst:results.image_namespace_build}.
...
\tcode{\$BaseName} is the filename of the container file and therefore the name of the image. The created temporary file \textit{container.tar} after the script has finished.
...
The official documentation of \ac{K8s} is incomplete or outdated on some pages. There is partially work in progress and while consuming the documentation it is hard to detect if a specific article is applicable for the current version of \ac{K8s}.
For example, this became fundamental on the page \enquote{Adding Windows nodes}\cite{Kubernetes.20220419}. At the current time being, the actual page is only available as an archived version for \ac{K8s} version \enquote{v1.23}. However, if the same \ac{URL} is called for the unarchived the user gets redirected to a similar page \enquote{Creating a cluster with kubeadm}\cite{Kubernetes.20221107}. Reading the page, people could assume that they can follow the steps described to add a \ac{Windows} node. However, the fact that the guide is aligned to Linux is only barely mentioned and thus is misleading.
That also means that there is no guide for adding \ac{Windows} nodes in the current version of the documentation of \ac{K8s}.

The guide for adding \ac{Windows} nodes provided from \ac{SIG} \enquote{Windows Tools}\cite{GitHubKubernetesSIGWindowsTools.20230213} has a more streamlined approach and benefits from provided PowerShell scripts, like \enquote{PrepareNode.ps1}. However, due to the small community, the guide still stays faulty or outdated. For example, the guide shows a warning, \enquote{The instructions and scripts in the directory DO NOT configure a CNI solution for Windows nodes running containerd. There is a work-in-progress PR to assist in this at \#239}\cite{GitHubKubernetesSIGWindowsTools.20230213}. However, the guide does explain how to configure a \ac{CNI} and the linked pull request is merged since December 2022.
...
The guide from \ac{SIG} \enquote{Windows Tools} was still work in progress while the cluster was setup. A change in the documentation for applying a required daemon set on the cluster was only added after some time. Taking into account that Windows nodes are supported since over year, a functional documentation was expected.

\section{Interfaces to other systems}
